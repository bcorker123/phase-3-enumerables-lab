...
def spiciest_foods(spicy_foods)
  spicy_foods.filter do |food_hash|
...
def sort_by_heat(spicy_foods)
  spicy_foods.sort do |food_hash1, food_hash2| 
    food_hash1[:heat_level] <=> food_hash2[:heat_level]
  end
end
...
def average_heat_level(spicy_foods)
  ...
  spicy_foods.map do |food_hash|
    heat_levels_array << food_hash[:heat_level]
  end

  (heat_levels_array.sum) / heat_levels_array.length 
end